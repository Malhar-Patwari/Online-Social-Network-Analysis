...
def token_pair_features(tokens, feats, k=3):
    ...
    for i in range(len(tokens)-k+1):
        #print(i)
        all_combinations = combinations(tokens[i:i+k], 2)
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    if vocab != None:      
      for doc_id in range(len(tokens_list)):
        #print(doc_id)  
        features_dict = dict(featurize(tokens_list[doc_id],feature_fns))
        #print(features_dict)        
        for feature in features_dict:
          #print(feature)
          ll_label.append(feature)
          resulted_vocabulary.append(feature)
          #print(resulted_vocabulary)
          if feature in vocab:
            ll_label.append(feature)       
            resulted_vocabulary.append((index,feature))                           
            data.append(features_dict[feature])
            field.append(doc_id)
            att.append(vocab[feature])
      #print(resulted_vocabulary)
      resulted_vocabulary=vocab
      ll_label.append(doc_id)

      csr = csr_matrix((data,(field,att)), shape=(len(tokens_list),len(resulted_vocabulary)))
      return csr, resulted_vocabulary
    

    elif vocab == None:
      new_vocab = defaultdict(list)
      document_map = defaultdict(dict)

      for doc_id in range(len(tokens_list)):
        #print(doc_id)
        ll_label.append(doc_id)
        feats = featurize(tokens_list[doc_id], feature_fns)
        features_dict = dict(feats)
        #print(features_dict)
        document_map[doc_id] = features_dict
        for feature in features_dict:
          #print(feature)          
          new_vocab[feature].append(doc_id)
          resulted_vocabulary.append(index)   
          ll_label.append(feature)
      ...
      for i in sorted(modified_vocabulary.keys()):
        #print(i)
        for doc_id in sorted(new_vocab[i]):
          #print(doc_id)
          ll_label.append(doc_id)
          if i in document_map[doc_id]:
            resulted_vocabulary.append((index,i))
            data.append(document_map[doc_id][i])
            field.append(doc_id)
            att.append(modified_vocabulary[i])
            ll_label.append(i)
      resulted_vocabulary=modified_vocabulary      

      csr = csr_matrix((data, (field, att)), shape=(len(tokens_list), len(modified_vocabulary)))
      return csr, resulted_vocabulary
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    for train, test in kfold:
      ...
      touple=accuracy_score(labels[test], predicted)
      all_accuracy.append(touple)
    #print(all_accuracy)  
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    feature_list=[]
    tokens_list=[]
    for feat in range(1, len(feature_fns)+1):
      #print(feat)
      for f in combinations(feature_fns, feat):
        #print(f)
    ...
    for punct in punct_vals:
      ...
      #print(token_list)  
      for feat1 in feature_list:
        #print(feat1)
        for min_freq in min_freqs:
          #print(min_freq)
          result_dict = {}
          X, vocab = vectorize(tokens_list, feat1, min_freq)
          avg = cross_validation_accuracy(LogisticRegression(), X, labels, 5)
          result_dict['features'] = feat1
          result_dict['punct'] = punct              
          result_dict['accuracy'] = avg
          result_dict['min_freq'] = min_freq              
...
def mean_accuracy_per_setting(results):
    ...
    setacc = defaultdict(lambda: 0)
    #print(results)
    #print(results)
    for i in results:
      #print(i)
      f_name = "features="
      setfreq['punct'+'='+str(i['punct'])]+= 1
      setacc['punct'+'='+str(i['punct'])]+= i['accuracy']              
      for func in i['features']:
        #print(func)
        f_name+=" "+func.__name__
      setfreq[f_name]+= 1
      setacc[f_name]+=i['accuracy']   
      #print(setacc)
      #print(setfreq)   
      setacc['min_freq'+'='+str(i['min_freq'])]+=i['accuracy']
      setfreq['min_freq'+'='+str(i['min_freq'])]+= 1
      #print(setacc)
    #print(setacc)
    #print(setfreq)  
    for i in setacc:
      #print(k)
      setacc[i] = setacc[i]/setfreq[i]
    #print(setacc)
    tuples=[]
    setacc=sorted(setacc.items(), key = lambda x: -x[1])
    for key,value in setacc:
...
def fit_best_classifier(docs, labels, best_result):
    ...
    for i in docs:
      #print(i)
      tok=tokenize(i,best_result['punct'])
      tokens.append(tok)
    #print(token_list)      
...
def top_coefs(clf, label, n, vocab):
    ...
    if label == 0:
      for i in vocab:
        #print(i)
        ans.append((i, clf.coef_[0][vocab[i]]))
        #print(ans)
      ...
      for i in vocab:
        ...
        ans.append((i,clf.coef_[0][vocab[i]]))
        #print(ans)
      ans= sorted(ans,key=lambda x:-x[1])[:n]
      return ans 
...
def parse_test_data(best_result, vocab):
    ...
    for d in test_docs:
      #print(d)
      tok=tokenize(d, best_result['punct'])
      tokens.append(tok)
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    predict_lbl = clf.predict(X_test)
    different_lbl = np.where(predict_lbl != test_labels)[0] 
    #print(different_lbl)
    predict_probability = clf.predict_proba(X_test)
    wrong_predicted = predict_probability[different_lbl]
    #print(wrong_predicted)
    wrong_ids = np.argsort(np.amax(wrong_predicted, axis = 1))[::-1][:n]
    #print(wrong_ids)
    for i in range(0, n):
      #print(i)
      doc_ids = different_lbl[wrong_ids[i]]
      #print(doc_ids)
      truth=str(test_labels[doc_ids])
      predicted=str(predict_lbl[doc_ids])
      probab=str(np.max(predict_probability[doc_ids]))
