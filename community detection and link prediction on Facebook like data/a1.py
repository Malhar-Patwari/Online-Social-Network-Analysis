...
# In this we'll implement community detection and link prediction algorithms using Facebook "like" data.
#
...
# For DISTANCE      
    ...
    for i in neighborss:
      if i == root:
        neighborss.extend(graph.neighbors(i))
        visited.append(i)
        numbered[i]=0      
        count = 1        
        if count > max_depth:
...
# FOR PARENTS              
    ...
    for i in neighborss:      
      current_neibors = graph.neighbors(i)  
      for j in current_neibors:
        if j not in numbered:
          continue  
        if (numbered[j] > numbered[i]) & (numbered[j]<=max_depth):          
...
# Number of shortest paths
    ...
    cur_neighbors = deque([root])
    level = 0
    temperory_list = []


    while level<= max_depth:      
      if len(traversed) == len(neighborss):
        break
      
      for i in cur_neighbors:        
        if level == int(numbered[i]):
          if i not in traversed:
            traversed.append(i)                      
          num_paths[i] +=1            
          temperory_list.extend(graph.neighbors(i))          
      cur_neighbors.clear()
      cur_neighbors.extend(temperory_list)
      del temperory_list[:]
      level+=1    

    #print("neighbors = ",neighborss)
    return numbered,num_paths,parents_dict
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    ###TODO      
    node_credit = defaultdict(lambda:1)
    edge_credit = {}

    for i in reversed(list(node2distances)):
      if i not in node2parents:
        break            
      num_parents = len(node2parents[i])
      for j in node2parents[i]:
        if i<j:            
          edge_credit[(i,j)] = float(node_credit[i]/num_parents)          
          node_credit[j]+=edge_credit[(i,j)]
        else:
          edge_credit[(j,i)] = float(node_credit[i]/num_parents)          
          node_credit[j]+=edge_credit[(j,i)]                              
...
def approximate_betweenness(graph, max_depth):
    ...
    for i in nodes:         
      ...
      edge_credit = bottom_up(i, node2distances, node2num_paths, node2parents)  
      c.update(edge_credit)      
...
def partition_girvan_newman(graph, max_depth):
    ...
    betweenness = sorted(sorted(approximate_betweenness(copy_graph, max_depth).items()), key=lambda x: (-x[1],x[0]))    
    all_edges = copy_graph.edges()
    for i in betweenness:           
      num_components = nx.number_connected_components(copy_graph)      
      if num_components == 1:        
        if i[0] in all_edges:
          copy_graph.remove_edge(*i[0])                
      else:
        break          
                    
    x = list(nx.connected_component_subgraphs(copy_graph))
...
def norm_cut(S, T, graph):
    ...
    normalized = ((cut_value/vol_s) + (cut_value/vol_t))    
    return normalized
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:            
      ...
      S = set(components[0].nodes())
      T = set(components[1].nodes())
      value = norm_cut(S,T,graph)
      list_score.append((i,value))      
    return list_score    
...
def make_training_graph(graph, test_node, n):
    ...
    count = 1
    copy_graph = graph.copy()
    neighbors = copy_graph.neighbors(test_node)
    for i in sorted(neighbors):
      if count <= n:
        copy_graph.remove_edge(test_node,i)
        count+=1    
    return copy_graph
...
# Number of shortest paths
    ...
    for i in neighborss:      
      if root<i:
        if ((root,i) not in all_edges):                    
          path_score = (beta**numbered[i])*num_paths[i]
          path.append(((root,i),path_score))
          
      else:
        if ((i,root) not in all_edges) & (root != i):
          path_score = (beta**numbered[i])*num_paths[i]
          path.append(((root,i),path_score))
              
    return sorted(path,key=lambda x: (-x[1],x[0]))[:k]
...
def evaluate(predicted_edges, graph):
    ...
    actual_edges = [x for x in predicted_edges if graph.has_edge(*x)]
    #print(actual_edges)
    res = len(actual_edges)/len(predicted_edges)
    #print(res)
    return res
    """
    ###TODO
